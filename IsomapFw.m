function [Y, R, E] = IsomapFw(D, n_fcn, n_size,options); 
...
M=500;
     ...
     if ~isempty(twod)
         figure;
         hold on;
         gscatter(Y.coords{twod}(1,1:M), Y.coords{twod}(2,1:M),labels_shuffled(1:M));
         gscatter(Y.coords{twod}(1,M+1:2*M), Y.coords{twod}(2,M+1:2*M),labels_shuffled(M+1:2*M));
         gscatter(Y.coords{twod}(1,2*M+1:end), Y.coords{twod}(2,2*M+1:end),labels_shuffled(2*M+1:end));
         if (overlay == 1)
