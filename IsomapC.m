function [Y, R, E] = IsomapC(D, n_fcn, n_size,options); 
...
if (displ==1)
     ...
     if ~isempty(twod)
         figure;
         hold on;
         scatter(Y.coords{twod}(1,1:600), Y.coords{twod}(2,1:600), 10, 'b', 'filled');
         scatter(Y.coords{twod}(1,601:end), Y.coords{twod}(2,601:end), 10, 'r', 'filled');
         if (overlay == 1)
